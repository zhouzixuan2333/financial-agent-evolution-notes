...
如果把 429、认证失败、空回答和业务错误都记成 0 分，模型排名会被基础设施状态污染。每条结果应同时有 `status` 和 `business_score`：先确认执行成功，再讨论模型质量。
...
| 模型规模 | SFT 阶段 Average F1 | 后训练阶段 Average F1 | 提升 |
| --- | ---: | ---: | ---: |
| 4B | 63.28 | 67.97 | +4.69 个百分点 |
| 35B | 72.06 | 73.81 | +1.75 个百分点 |

这组数字适合说明“后训练在统一总览口径中继续带来收益”，但不能反推出每个 Skill 都提升，也不能与不同 Runtime、不同 Skill 集合或后续版本的明细表混成一条曲线。

小模型绝对增益更大，符合一个合理但仍需验证的解释：小模型距离教师或任务最优策略更远，可纠正空间更大；大模型 SFT 起点较高，平均提升更容易被少数弱项抵消。
