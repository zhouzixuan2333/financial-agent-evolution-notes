...
严格比较指标 ID 会判错，因为模型没有返回同一个指标。但三个月的月度数据可以直接求和，业务上已经足以回答问题。

因此，评测需要判断的不是“预测是否和标准答案一模一样”，而是：

> 当前返回的数据，能不能可靠地回答用户问题？

我使用 LLM-as-a-Judge 补充判断指标名称、频率、单位、时间范围和可计算关系，并通过人工争议 Case 迭代判定规则。独立样本复测中，Judge 与人工判定一致率达到 93%。
...
平均正确率提升 17.84 个百分点，说明传统检索链路仍然存在明显的工程优化空间。
...
Top10 与 Top5 只差 0.22 个百分点，继续扩大到 Top40 也没有更好。这意味着继续增加候选数量的边际收益已经很低，更多候选反而可能增加筛选噪声。
...
4. TopK 敏感性不仅用于调参，也能帮助判断当前技术路线是否接近能力上限。
