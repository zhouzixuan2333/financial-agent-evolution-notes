...
数据背景见[底层数据与目录结构](00-project-map.md#0-底层数据与目录结构)：召回阶段找候选指标，取数阶段按选定 ID 和时间范围获取数值。
...
下面用一个合成例子说明：用户问“某商品第一季度的总销量是多少？”

| 标准答案 | 模型返回 | 判断 |
| --- | --- | --- |
| 季度总销量：90 万件 | 1 月 25、2 月 30、3 月 35 万件 | 指标 ID 不同，但月度值相加为 90 万件，仍能回答问题 |

这里要求三个数都是同口径的当月值，且没有缺月或重复；累计值不能直接相加。只比较指标 ID，会把这种可计算的答案误判为错误。

因此，我建立 LLM-as-a-Judge，把原问题、参考答案和实际返回的指标与数据一起交给模型，检查名称、频率、单位、时间范围及可计算关系，给出判定和理由。本例的理由就是：“三个月数据完整、口径一致，可以求和得到季度销量。”

建立初版规则后，我对照人工争议 Case 修正规则：哪些是等价取数，哪些只是看起来相关。后续独立 100 条复测中，Judge 与人工判定一致率达到 93%。有了这把尺子，新旧 Workflow 才能统一比较，失败原因也能回流到后续优化中。
...
原链路：多维向量召回 → 综合打分取 Top1 → 执行取数
选中：该国该商品进口总量
问题：返回的是所有贸易伙伴的总量，遗漏了“贸易伙伴 A”的限制。
```
...
沿用上一节的进口问题，优化后的处理可以这样理解（合成示意）：

```text
保留原问题，展开“从 A 进口数量”“从 A 进口年度累计量”等查询表达
→ 多类接口并行召回候选
→ LLM 排除“所有贸易伙伴总量”“进口金额”等不符口径的结果
→ 选取从 A 进口的数量指标，核验年份与单位后组织答案
→ Judge 检查返回数据是否满足原问题
```

两条链路的区别在于：原链路直接采用综合得分最高的候选；优化后用原问题的完整条件筛选候选，再检查结果能否回答问题。
